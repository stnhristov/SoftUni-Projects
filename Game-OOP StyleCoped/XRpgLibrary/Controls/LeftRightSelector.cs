namespace XRpgLibrary.Controls
{
    ...
    public class LeftRightSelector:Control
    {
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            float itemWidth = base.SpriteFont.MeasureString(items[selectedItem]).X;
            float offset = (maxItemWidth - itemWidth) / 2;

            drawTo.X += offset;

            if (base.HasFocus) spriteBatch.DrawString(base.SpriteFont, items[selectedItem], drawTo, selectedColor);
            else spriteBatch.DrawString(base.SpriteFont, items[selectedItem], drawTo, Color);
